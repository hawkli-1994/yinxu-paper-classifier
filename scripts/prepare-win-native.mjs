...
const targetDirectory = join(projectRoot, 'node_modules', '@napi-rs', 'canvas-win32-x64-msvc');
const bindingPath = join(targetDirectory, 'skia.win32-x64-msvc.node');
const icuDataPath = join(targetDirectory, 'icudtl.dat');
const packageJsonPath = join(targetDirectory, 'package.json');

const isPrepared = () => {
  if (!existsSync(bindingPath) || !existsSync(icuDataPath) || !existsSync(packageJsonPath)) return false;
  try {
...
const temporaryDirectory = await mkdtemp(join(tmpdir(), 'yinxu-win-native-'));
try {
  ...
  console.log(`Prepared ${packageSpec} for Windows x64 packaging.`);
} finally {
