...
export const HistoryPage = (): React.JSX.Element => {
  ...
  const activate = async (revisionId: string): Promise<void> => {
    try {
      ...
      message.success('已将所选结果设为当前版本，其他版本仍会保留。');
    } catch (error) {
  ...
  return (
    <section className="workspace-section">
      <Alert type="info" showIcon className="section-alert compact-alert" message="每次 AI 分类和人工复核都会生成独立版本，不覆盖已有结果。系统同时保留所用模型、分类知识库和补充材料的版本信息。" />
      <Row gutter={[16, 16]}>
        <Col xs={24} xl={11}>
          <Card title={`分类任务（${workspace.runs.length}）`} className="academic-card">
            <List
              locale={{ emptyText: '暂无分类任务' }}
                ...
                return (
                  <List.Item actions={[active ? <Tag key="current" color="green">当前版本</Tag> : <Button key="activate" type="link" onClick={() => void activate(revision.id)}>设为当前版本</Button>]}>
                    <List.Item.Meta
                      ...
                      title={<Space wrap><Typography.Text strong>版本 {workspace.revisions.length - index}</Typography.Text><Tag color={revision.kind === 'review' ? 'blue' : 'default'}>{revision.kind === 'review' ? '人工复核结果' : 'AI 分类结果'}</Tag></Space>}
